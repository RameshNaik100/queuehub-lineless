...
function json(res, status, payload) { const body = JSON.stringify(payload); res.writeHead(status, { 'Content-Type':'application/json; charset=utf-8', 'Cache-Control':'no-store', 'Access-Control-Allow-Origin':'*', 'Access-Control-Allow-Headers':'Content-Type', 'Access-Control-Allow-Methods':'GET,PUT,POST,OPTIONS' }); res.end(body); }
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS services (id TEXT PRIMARY KEY, organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE, slug TEXT NOT NULL, name TEXT NOT NULL, description TEXT, category TEXT, prefix TEXT, published INTEGER NOT NULL DEFAULT 0, paused INTEGER NOT NULL DEFAULT 0, demo_open INTEGER NOT NULL DEFAULT 0, schedule_json TEXT NOT NULL, created_at TEXT NOT NULL, UNIQUE(organization_id, slug));
...
function upsertService(org, dbOrg, incoming) {
  ...
  db.prepare(`INSERT INTO services(id,organization_id,slug,name,description,category,prefix,published,paused,demo_open,schedule_json,created_at) VALUES(@id,@orgId,@slug,@name,@description,@category,@prefix,@published,@paused,@demoOpen,@schedule,@createdAt)
    ON CONFLICT(id) DO UPDATE SET slug=excluded.slug, name=excluded.name, description=excluded.description, category=excluded.category, prefix=excluded.prefix, published=excluded.published, paused=excluded.paused, demo_open=excluded.demo_open, schedule_json=excluded.schedule_json`).run({ id:service.id || uid('svc'), orgId:dbOrg.id, slug:service.slug, name:service.name, description:service.description || '', category:service.category || '', prefix:service.prefix || String(service.name || 'Q').slice(0,1).toUpperCase(), published:service.published ? 1 : 0, paused:service.paused ? 1 : 0, demoOpen:service.demoOpen ? 1 : 0, schedule:JSON.stringify(schedule), createdAt:service.createdAt || new Date().toISOString() });
  ...
  for (const token of (service.tokens || [])) {
    ...
    db.prepare(`INSERT OR IGNORE INTO queue_entries(id,service_id,service_date,number,token,customer_name,phone,status,counter,joined_at,started_at,completed_at,service_minutes,priority_requested,priority_approved,last_recalled_at)
      VALUES(@id,@serviceId,@date,@number,@token,@customerName,@phone,@status,@counter,@joinedAt,@startedAt,@completedAt,@serviceMinutes,@priorityRequested,@priorityApproved,@lastRecalledAt)`).run({ id:token.id || uid('tok'), serviceId:dbService.id, date:token.date || todayISO(), number:Number(token.number || 0), token:token.token, customerName:token.customerName || 'Customer', phone:token.phone || '', status:token.status || 'waiting', counter:token.counter || null, joinedAt:token.joinedAt || new Date().toISOString(), startedAt:token.startedAt || null, completedAt:token.completedAt || null, serviceMinutes:token.serviceMinutes || null, priorityRequested:token.priorityRequested ? 1 : 0, priorityApproved:token.priorityApproved ? 1 : 0, lastRecalledAt:token.lastRecalledAt || null });
  }
  return dbService;
}
...
function resetDemoEntries(broadcastAfter=true) {
  ...
  const work = () => {
    ...
    rows.forEach((item,index)=>db.prepare(`INSERT INTO queue_entries(id,service_id,service_date,number,token,customer_name,phone,status,counter,joined_at,started_at,completed_at,service_minutes,priority_requested,priority_approved) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(`tok_ramesh_${index+1}`,service.id,date,index+1,item[0],item[1],`91000000${index}`,item[2],item[2]==='called'?1:null,now,item[2]==='called'?now:null,null,null,0,0));
...
async function handleApi(req,res,url) {
  if (req.method==='OPTIONS') { res.writeHead(204,{'Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Content-Type','Access-Control-Allow-Methods':'GET,PUT,POST,OPTIONS'}); return res.end(); }
  ...
  try {
    ...
    if (req.method==='PUT' && !operation) { const body=await readBody(req); if(!body.organization?.slug || !body.service?.slug) return json(res,400,{error:'Organization and service are required'}); const record=runTransaction(()=>{ const org=upsertOrganization(body.organization); upsertUsers(body.organization,org); upsertService(body.organization,org,body.service); return recordBySlugs(org.slug,body.service.slug); }); broadcast(orgSlug,serviceSlug); return json(res,200,record); }
    if (req.method==='POST' && operation==='join') {
      const body=await readBody(req); const result=runTransaction(()=>{
        ...
        const next=db.prepare('SELECT COALESCE(MAX(number),0)+1 AS number FROM queue_entries WHERE service_id=? AND service_date=?').get(service.id,date).number; const prefix=service.prefix||service.name.slice(0,1).toUpperCase(); const token={id:uid('tok'),date,number:next,token:`${prefix}${String(next).padStart(3,'0')}`,customerName,phone,status:'waiting',counter:null,joinedAt:new Date().toISOString(),startedAt:null,completedAt:null,serviceMinutes:null,priorityRequested:body.priorityRequested===true,priorityApproved:false}; db.prepare(`INSERT INTO queue_entries(id,service_id,service_date,number,token,customer_id,customer_name,phone,status,counter,joined_at,started_at,completed_at,service_minutes,priority_requested,priority_approved) VALUES(@id,@serviceId,@date,@number,@token,@customerId,@customerName,@phone,@status,@counter,@joinedAt,@startedAt,@completedAt,@serviceMinutes,@priorityRequested,@priorityApproved)`).run({id:token.id,serviceId:service.id,date,number:token.number,token:token.token,customerId:customer.id,customerName,phone,status:'waiting',counter:null,joinedAt:token.joinedAt,startedAt:null,completedAt:null,serviceMinutes:null,priorityRequested:token.priorityRequested?1:0,priorityApproved:0}); notify(org.id,service.id,token.id,'joined',`Token ${token.token} joined the queue.`); history(org.id,service.id,token.id,'joined',{token:token.token}); notifyQueueSignals(org.id,service.id,date); return {status:200,body:{token,...recordBySlugs(orgSlug,serviceSlug)}};
